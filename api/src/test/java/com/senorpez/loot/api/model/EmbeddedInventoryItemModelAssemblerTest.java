...
class EmbeddedInventoryItemModelAssemblerTest {
    ...
    void serialize_model() throws JsonProcessingException, JSONException {
        ...
        InventoryItemEntity entity = spy(new InventoryItemEntity(inventoryItem, item));
        when(entity.getId()).thenReturn(expectedId);

        EmbeddedInventoryItemModelAssembler assembler = new EmbeddedInventoryItemModelAssembler();
        ...
        final String expectedJson = String.format(
                "{name: \"%s\", weight: %.2f, charges: %d, details: \"%s\", quantity: %d}",
                expectedName, expectedWeight, expectedCharges, expectedDetails, expectedQuantity
        );
