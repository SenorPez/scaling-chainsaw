...
/*
TODO:
 This is currently the same as additem.js,
 but with a change to how quantity is set.
 Refactor for DRY.
 */
    ...
    if (matches[0]) {
        if (matches[0][3]) {
            ...
            if (argmatches[0]) {
                ...
                message.channel.send('Usage: $dropitem &lt;quantity&gt; &lt;item name&gt; [--r &ltremark&gt]');
                return;
            }
        }
    } else {
...
function postTransaction(message, item, quantity, accessToken, args) {
    ...
    fetch(`https://www.loot.senorpez.com/campaigns/${state.getCampaignId()}/characters/${state.getCharacterId()}/itemtransactions`, {
        ...
        .then(response => response.json())
        .then(data => {
            message.channel.send(`Dropped ${-quantity} ${item.name} from ${data.name}`);
